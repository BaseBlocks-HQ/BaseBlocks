...
import { createLayout, generateId } from "@/modules/editor/layout";
...
export function PageEditor({
  ...
  const handleMainLayoutDragEnd = async (event: {
    ...
    await reorderLayouts(mainLayoutIds, sidebarLayoutIds, oldIndex, newIndex);
  };
  ...
  const handleSidebarLayoutDragEnd = async (event: {
    ...
    await reorderLayouts(sidebarLayoutIds, mainLayoutIds, oldIndex, newIndex);
  };
  ...
  const handleAddLayout = async (type: LayoutType) => {
    ...
    if (firstSlot) {
      setTimeout(() => {
        ...
        onSelectionChange?.(firstSlot.id);
      }, 100);
    }
  };
  ...
  const handleRemoveLayout = async (layoutId: string) => {
    await removeLayoutMutation({ layoutId: layoutId as Id<"layouts"> });
    clearSelection();
  };
  ...
  const handleUpdateSettings = async (
    ...
    await updateLayoutSettingsMutation({
      layoutId: layoutId as Id<"layouts">,
      settings,
    });
  };
  ...
  const handleDisableTabs = async () => {
    await disablePageTabsMutation({ pageId: pageId as Id<"pages"> });
    setActiveTabId(null);
  };

  const handleAddTab = async () => {
    const tab = { id: generateId(), label: `Tab ${pageTabs.length + 1}` };
