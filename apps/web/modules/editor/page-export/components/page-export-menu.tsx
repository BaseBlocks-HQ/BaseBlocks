...
export function PageExportMenu({
  ...
  const activeFormat = exportFormats[0];
  const hasMultipleFormats = exportFormats.length > 1;

  if (!hasMultipleFormats && !trigger) {
    return (
      <Button
        variant="outline"
        size="sm"
        title={`Export as ${activeFormat.label}`}
        className="h-8 gap-2"
        onClick={activeFormat.onSelect}
      >
        <span className="text-muted-foreground">Export as</span>
        <span className="inline-flex items-center gap-1.5 font-medium text-foreground">
          <WordLogoIcon className="h-4 w-4 shrink-0" />
          {activeFormat.label}
        </span>
      </Button>
    );
