...
import { getAccessSessionCookieName } from "@/features/published-sites/access-session";
...
function setCookie(name: string, value: string, days: number): void {
  ...
  // biome-ignore lint/suspicious/noDocumentCookie: synchronous cookie access is required before retrying the gated request.
  document.cookie = `${name}=${value}; expires=${expires.toUTCString()}; path=/; SameSite=Lax${secure}`;
}

function removeCookie(name: string): void {
  if (typeof document === "undefined") return;
  // biome-ignore lint/suspicious/noDocumentCookie: this must synchronously clear the access token.
